...
<b><details><summary>匿名管道</summary></b>
...
<b><details><summary>命名管道</summary></b>

- **特点**
    - 命名管道是 `FIFO` 文件，存在于文件系统中，可以通过文件路径名来指出
    - 命名管道可以在任意关系的进程间通信
- **优点**
    - 可以实现任意关系的进程间的通信
- **缺点**
...
- **函数调用**
    ...
    参数：
        path 即将创建的FIFO文件路径，如果文件存在需要先删除
        mode 和 open() 中参数相同，即规定FIFO文件的访问权限
    返回值：
        0  成功
        -1 失败，并返回 error 错误码
    ```
...
- **优点**
    - 无需复制，快捷，信息量大
- **缺点**
    - 通信是通过将共享空间缓冲区直接附加到进程的虚拟空间来实现的，因此进程间有读写操作的同步问题
...
- **优点**
    - 可以实现任意进程间的通信，并通过系统调用函数来实现发送和接收之间的同步，无需考虑同步的问题，方便
- **缺点**
    - 信息的复制需要消耗额外的 `CPU` 时间，不适宜信息量大或操作频繁的场合
